...
app.use(cookieParser()); //ADDED/FIXED
...
app.post("/auth/token", (req, res) => {
  try {
    const sessionId = req.cookies.session_token; // ← FIXED: was reading from Authorization header
    ...
    const accessToken = jwt.sign(
      {
        email: session.email,
        sessionId: sessionId, // ← FIXED: clean variable
      },
      secret,
      { expiresIn: "15m" }
    );
